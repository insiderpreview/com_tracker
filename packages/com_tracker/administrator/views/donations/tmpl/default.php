	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_TRACKER_DONATION_UID', 'du.username', $listDirn, $listOrder); ?></th>

				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_TRACKER_DONATION_DONATED', 'a.donated', $listDirn, $listOrder); ?></th>

				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_TRACKER_DONATION_RATIO', 'a.ratio', $listDirn, $listOrder); ?></th>

				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_TRACKER_DONATION_CREDITED', 'a.credited', $listDirn, $listOrder); ?></th>

				<th class='left'><?php echo JHtml::_('grid.sort',  'JGLOBAL_CREATED_DATE', 'a.created_time', $listDirn, $listOrder); ?></th>

				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_TRACKER_DONATION_USERNAME', 'u.username', $listDirn, $listOrder); ?></th>

				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_TRACKER_DONATION_COMMENTS', 'a.comments', $listDirn, $listOrder); ?></th>

				<th width="10%">
					<?php echo JHtml::_('grid.sort',	'JGRID_HEADING_ORDERING', 'a.ordering', $listDirn, $listOrder); ?>
					<?php if ($saveOrder) :?>
						<?php echo JHtml::_('grid.order',	$this->items, 'filesave.png', 'donations.saveorder'); ?>
					<?php endif; ?>
				</th>
		...
		<?php foreach ($this->items as $i => $item) :
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
				<?php if ($canEdit) { ?>
					<a href="<?php echo JRoute::_('index.php?option=com_tracker&task=donation.edit&id='.(int) $item->id); ?>">
					<?php echo $item->donator; ?>
					</a>
					<?php } else { ?>
					<?php echo $item->donator; ?>
				<?php } ?>
				...
				<td class="center">
					<?php echo JHtml::_('grid.boolean', $i, $item->state, 'donations.publish', 'donations.unpublish'); ?>
				</td>
